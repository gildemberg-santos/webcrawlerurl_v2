...
`))
	...
	assert.Equal(t, "https://www.teste.com/ah0116002-sandalia-basica-couro-bege-detalhe-laser/p", response.Data[0].Url)
	assert.Equal(t, "https://www.teste.com/ah0116001-sandalia-basica-couro-preta-detalhe-laser/p", response.Data[1].Url)
}

func TestEcommerce_NotNormalizeUrl(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()

	httpmock.RegisterResponder("GET", "https://www.teste.com/ah0116001-sandalia-basica-couro-preta-detalhe-laser/p?teste=123", httpmock.NewStringResponder(200, `
		<!DOCTYPE html>
			<head>
				<title>Titulo do site</title>
				<meta property="product:retailer_item_id" content="AH0116001" data-react-helmet="true">
...
`))
	...
	httpmock.RegisterResponder("GET", "https://www.teste.com/ah0116002-sandalia-basica-couro-bege-detalhe-laser/p?teste=123", httpmock.NewStringResponder(200, `
		<!DOCTYPE html>
			<head>
				<title>Titulo do site</title>
				<meta property="product:retailer_item_id" content="AH0116002" data-react-helmet="true">
...
`))
	...
	assert.Equal(t, int64(38), response.TotalCaracters)
	assert.Equal(t, 2, len(response.Data))
	assert.Equal(t, "https://www.teste.com/ah0116002-sandalia-basica-couro-bege-detalhe-laser/p?teste=123", response.Data[0].Url)
	assert.Equal(t, "https://www.teste.com/ah0116001-sandalia-basica-couro-preta-detalhe-laser/p?teste=123", response.Data[1].Url)
}
